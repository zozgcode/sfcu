    ...
    <div className="px-[16px]">
      <div className="w-full">
        <div className="text-[14px] w-full font-[500] flex items-center justify-between">
          ...
          <Link href="/dashboard/transactions" className="text-[13px] font-[500] text-[#00767d]">See More</Link>
        </div>
        <div className="mt-[10px]">
          {transactionsToShow.map((transaction: Transaction) => (
            <div key={transaction.transaction_id} className="flex justify-between py-3">
              ...
              <button className="border-none flex items-center justify-center outline-none rounded-full w-[35px] h-[35px] bg-[#00767d]/10"><HiArrowDown className={`${transaction.amount_usd < 0 ? "rotate-180" : ""}`} /></button>
              ...
              <div className="flex flex-col gap-1 justify-between text-right">
                ...
                <span className={`text-[10px] font-medium ${transaction.status === "Failed" ? "text-red-600" : "text-green-600"}`}>{transaction.status}</span>
              </div>
            </div>
          ))}
        </div>
